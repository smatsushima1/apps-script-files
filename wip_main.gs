...
function compareData() {
  ...
  var data_ww = getDataValues(_ga_ww, 0, 0);
  ...
  var data_nwc = getDataValues(_ga_nwc, 0, 0);
  var data_nww = getDataValues(_ga_nww, 0, 0);
  // Columns to pull data from
...
function updateAllBranchWIPs(curr_fy_ind) {
  ...
  var data_nwc = getDataValues(_ga_nwc, -1, 0);
  // Mailbox items
  var data_mi = [];
  for (var i = 0; i < data_cw.length; i++) {
...
function dev002() {
  var list = [["EAD-NOS MAILBOX", 1], ["EAD-NOS MAILBOX", 0], ["EAD-NWS MAILBOX", 2], ["EAD-NWS MAILBOX", 3], ["EAD-OMAO MAILBOX", 5], ["EAD-SAP MAILBOX", 2]];
  var new_list = [];
  for (var i = 0; i < list.length; i++) {
    new_list.push(list[i][0]);
  };
  console.log(new_list);
  console.log(new_list.filter(x => x == "EAD-OMAO MAILBOX").length);
};
...
function dev003() {
  var ga = SpreadsheetApp.openById("1jRFo2mSFQZpaYDZ68-SZB__TEYefyVxerrU8nYiO58c");
  var sht = ga.getSheetByName("Summary");
  console.log(getLastDataRow(sht, 32));
};
...
function updateImportData() {
  ...
  var cw_data = getDataValues(_ga_cw, 0, 0);
  var mb_data = [];
  ...
  // Add mailbox data
  console.log('Transfer Mailbox data');
  var nwc_data = getDataValues(_ga_nwc, 0, 0);
  var mb_data = [];
...
function cwUpdateWIP() {
  ...
  var ga_ww_values = getDataValues(_ga_ww, 0, 0);
  var ga_nww_values = getDataValues(_ga_nww, -1, 0);
...
function cwCreatePivots01(curr_fy_ind) {
  ...
  // Row 5
  frow = _gac_e.getLastRow();
  cwPivot01("Andrew Hildebrandt's OMAO Branch", _gac_e, drange, frow + 3, 1, 3, ['OMAO'], pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
  // Row 6
  frow = _gac_e.getLastRow();
...
function cwCreatePivots02(curr_fy_ind) {
  ...
  cwPivot01("Emily Clark's Total WIP", _gac_ec, drange, 1, 1, 23, filter_values, pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
  ...
  cwPivot01("Andrew Hildebrandt's OMAO Branch", _gac_ec, drange, frow + 3, 7, 3, ['OMAO'], pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
  // Row 4
  frow = _gac_ec.getLastRow();
  ...
  cwPivot01("Emily Clark's Summary", _gac_ec, drange, 1, 22, 1, filter_values, pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
  // Formatting
  changeFont(_gac_ec);
...
function cwCreatePivots03(curr_fy_ind) {
  ...
  cwPivot01("Nick DeGuire's Total WIP", _gac_nd, drange, 1, 1, 23, filter_values, pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
  ...
  cwPivot01("Nick DeGuire's Summary", _gac_nd, drange, 1, 22, 1, filter_values, pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
  // Formatting
  changeFont(_gac_nd);
...
function cwCreatePivots04(curr_fy_ind) {
  ...
  cwPivot02("Andrew Hildebrandt's OMAO Branch by Contract Type", _gac_sb, drange, 1, 13, ['OMAO'], fy_values, curr_fy_ind, [3]);
...
function cwCreatePivots05(curr_fy_ind) {
  ...
  cwPivot03("Andrew Hildebrandt's OMAO Branch by Contract Type", _gac_ss, drange, 1, 16, ['OMAO'], fy_values, curr_fy_ind, [4]);
